...
<body>
    <center>
        ...
        <%
            ...
            // 만약 처음 boardlist.jsp 를 클릭하거나 수정, 삭제 등 다른 게시글에서 이페이지로 넘어오면 pageNum 값이 없기에 null 로 처리(가장 최신의 페이지 보여줌)
            if (pageNum == null) {
                pageNum = "1";
            }
            ...
            // 최신글 10개를 기준으로 게시글을 반환하는 메소드 호출
            Vector<BoardBean> vec = bdao.getAllBoard(startRow, endRow);
            // 테이블에 표시할 번호 지정
            number = count - (currentPage - 1) * pageSize;
        %>
        <table width="700" border="1" bgcolor="#d3d3d3">
            <tr height="40">
            ...
            <%
                for (int i = 0; i < vec.size(); i++) {
                    BoardBean bean = vec.get(i); // 벡터에 저장된 빈 클래스 추출
            %>
            <tr height="40">
